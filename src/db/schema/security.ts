...
export const userRoles = createTable("user_roles", {
    ...
    roleId: varchar("role_id", {length:21}).notNull().references(() => roles.id),
}, (table) => [primaryKey({
    name: "user_roles_pk",
...
export const rolePermissions = createTable("role_permissions", {
    roleId: varchar("role_id", {length:21}).notNull().references(() => roles.id),
    permissionId: varchar("permission_id", {length:21}).notNull().references(() => permissions.id),
...
export const teamAccess = createTable("team_access", {
    userId: varchar("user_id", {length:21}).notNull().references(() => users.id),
...
export const organizationAccess = createTable("organization_access", {
    userId: varchar("user_id", {length:21}).notNull().references(() => users.id),
    organizationId: varchar("organization_id", {length:21}).notNull().references(() => organizations.id),
    accessLevel: varchar('access_level', { length: 20 }).notNull(),
}, (table) => [
    primaryKey({
...
export const securityLogs = createTable("security_logs", {
    id: varchar('id', {length: 21}).primaryKey().$defaultFn(() => createId()),
    userId: varchar("user_id", {length:21}).notNull().references(() => users.id),
    action: varchar('action', { length: 50 }).notNull(),
    resource: varchar('resource', { length: 50 }).notNull(),
